...
describe("createUpstream", () => {
  test("passes defaultHeaders through to every request", async (t) => {
  ...
  test("works without defaultHeaders (OMDb-style client)", async (t) => {
    const mock = mockFetch(() => jsonResponse({ ok: true }));
    installFetch(t, mock);
    const { http } = upstream();
  ...
  test("the cache stores and reuses values", async () => {
  ...
  test("gates requests through a rate limiter honoring minIntervalMs", async (t) => {
    const mock = mockFetch(() => jsonResponse({ ok: true }));
    installFetch(t, mock);
