...
- Disclose that `get_videos` is pinned to the server's `TMDB_LANGUAGE` with no per-call override, which can shrink its result set ([0d90dff](https://github.com/Grinv/tmdb-mcp/commit/0d90dff)).
...
- Add `include_adult` to `discover_movies`/`discover_tv` — verified live to change the result set on both endpoints ([7ed7c99](https://github.com/Grinv/tmdb-mcp/commit/7ed7c99)).
- Add `region` to `discover_movies` (movie-only) — its documented date-filtering effect wasn't reproducible live, only disclosed as-is ([7ed7c99](https://github.com/Grinv/tmdb-mcp/commit/7ed7c99)).
- Add `type` (`movie`/`series`/`episode`) to `get_ratings`'s title lookup, to disambiguate same-titled entries of different types (e.g. "Chuck" the movie vs. the series) ([6a35918](https://github.com/Grinv/tmdb-mcp/commit/6a35918)).
...
- Drop each episode's `overview` from `get_tv`'s `expand_episodes` aggregate, since the combined episode cap alone didn't keep the response usable; `get_tv_season` (single season) is unaffected ([b971897](https://github.com/Grinv/tmdb-mcp/commit/b971897)).
...
- Reject a malformed `with_original_language` up front instead of silently no-op'ing the `discover_movies`/`discover_tv` filter, same as `language` ([9af534d](https://github.com/Grinv/tmdb-mcp/commit/9af534d)).
- Fix `top_by_entity`'s TV step referencing a movies-filter explanation that isn't shown when `media_type=tv` ([8a2fa96](https://github.com/Grinv/tmdb-mcp/commit/8a2fa96)).
- Cap `get_tv`'s `expand_episodes` aggregate by actual serialized size instead of episode count alone, and stop fetching seasons already known to fall entirely beyond the budget ([6f171fa](https://github.com/Grinv/tmdb-mcp/commit/6f171fa)).
